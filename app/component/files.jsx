...
export default function Files ({ data, setData, multiple, user }) {
    ...
    const add_file = ( e ) => {
        ...
        Array.from(e.target.files).forEach((f) => {
            ...
            fr.onload = () => {
                ...
                let new_files = data.new_files;
                new_files.push({file: f, name: name, type: type, size: size, ext: ext});
                setData({...data, new_files: new_files});
    ...
    const del_file = ( index, id ) => {
        ...
        let _new_files_ = data.new_files.slice(0, index).concat(data.new_files.slice(index+1));
        setData({...data, new_files: _new_files_});
    ...
    useEffect(() => {
        ...
        if ( multiple ) {
            ...
            setFiles(data.files?.map(_ => [_[0], `${host}/P${_[1]}`, _[1]]) || []);
            setIndex(data.files?.length ? 1 : 0);
            setTimeout(layout, 500);
            window.addEventListener('click', function(e) { setTimeout(layout, 200) });
        }
        else {
            if ( user ) setImage(`${host}/U${data.id}`);
            else setImage(`${host}/C${data.id}`);
        }
        ...
        <Fragment>
            {
                ...
                <div className="mb-6 px-4 w-full">
                    ...
                    <div className="w-full over-hidden flex items-center text-black dark:text-white shrink-0 image-banner relative for-ar">

                        <Swiper modules={[Navigation]} navigation={true} onSwiper={setSwiper} onSlideChange={(e) => setIndex(e.realIndex+1)}>
                            {
                                files.length ? files.map((item, index) => 
                                    ...
                                    <SwiperSlide key={index}>
                                        ...
                                        {
                                            item[0] === 'image' ?
                                            <img src={item[1]} onError={(e) => e.target.src = '/media/public/error_icon.png'}/> : 
                                        ...
                                        <div className="actions absolute">
                                            ...
                                            <a href={item[1]} download target="_blank">
                                                <span className="material-symbols-outlined icon">arrow_downward</span>
                                            </a>

                                            <a onClick={() => item.length > 2 ? del_file(index, item[2]) : del_file(index)}>
                    ...
                    <div className="files-banner w-full flex items-center justify-between flex-wrap mt-5 no-select">

                        <div className="flex items-center justify-start">

                            <button type="button" className="btn btn-primary add-img-btn" onClick={() => inputFile.current.click()}>

                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-5 h-4 ltr:mr-2 rtl:ml-2">
                                    <path d="M3 15C3 17.8284 3 19.2426 3.87868 20.1213C4.75736 21 6.17157 21 9 21H15C17.8284 21 19.2426 21 20.1213 20.1213C21 19.2426 21 17.8284 21 15" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path>
                ...
                <div className={`over-hidden flex items-center text-black dark:text-white shrink-0 banner relative ${user ? 'user-banner' : ''}`}>

                    <div className='image layer-div'>

                        <img 
                            src={image} className="banner-image" 
                            onLoad={(e) => e.target.src.includes('_icon') ? e.target.classList.add('empty') : e.target.classList.remove('empty')}
                            onError={(e) => e.target.src = `/media/public/${user ? 'user' : 'error'}_icon.png`}
                        />

                    </div>

                    <div className="edit-image add-img-btn flex pointer absolute" onClick={() => inputFile.current?.click()}>
                    ...
                    <input type="file" ref={inputFile} onChange={change_image} className="hidden"/>
